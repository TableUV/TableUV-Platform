...
class SFEVL53L1X
{
	...
	uint16_t checkBootState(); //Check if the VL53L1X has been initialized
	...
	* To set the center, set the pad that is to the right and above the exact center of the region you'd like to measure as your opticalCenter*/
	VL53L1X_ERROR setROI(uint8_t x, uint8_t y, uint8_t opticalCenter); //Set the height and width of the ROI(region of interest) in SPADs, lowest possible option is 4. Set optical center based on above table
	VL53L1X_ERROR setCenter(uint8_t opticalCenter); //Set optical center based on above table
