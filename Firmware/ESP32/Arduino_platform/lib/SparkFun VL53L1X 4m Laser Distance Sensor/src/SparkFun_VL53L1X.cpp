...
uint16_t SFEVL53L1X::checkBootState()
{
	uint8_t bootState;
...
VL53L1X_ERROR SFEVL53L1X::setROI(uint8_t x, uint8_t y, uint8_t opticalCenter)
{
	return _device->VL53L1X_SetROI(x, y, opticalCenter);
}

VL53L1X_ERROR SFEVL53L1X::setCenter(uint8_t opticalCenter)
{
	return _device->VL53L1X_SetCenter(opticalCenter);
}
